	...
	if ($removeapps.length -eq 0){
		...
		foreach ($app in $removeapps) {
			...
			if ($isAppInstalled -ne $app){
				...
				#### ***KE2209001*** Package uninstall for all users throws error at the end. After removal from last local user package will move automatically to staged status/user ({S-1-5-18 [S-1-5-18]: Staged}), which is not removabe by "Remove-AppxPackage" command. Regardless error package is being removed for all local users."
			}
		}
	}
}
